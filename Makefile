...
CONFIG_PKG=github.com/AbdullahMalik17/malikclaw/pkg/config
...
MALIKCLAW_HOME?=$(HOME)/.malikclaw
WORKSPACE_DIR?=$(MALIKCLAW_HOME)/workspace
...
## build: Build the malikclaw binary for current platform
build: generate
...
## build-launcher: Build the malikclaw-launcher (web console) binary
build-launcher:
	@echo "Building malikclaw-launcher for $(PLATFORM)/$(ARCH)..."
	@mkdir -p $(BUILD_DIR)
	@if [ ! -f web/backend/dist/index.html ]; then \
		echo "Building frontend..."; \
		cd web/frontend && pnpm install && pnpm build:backend; \
	fi
	@$(GO) build $(GOFLAGS) -o $(BUILD_DIR)/malikclaw-launcher-$(PLATFORM)-$(ARCH) ./web/backend
	@ln -sf malikclaw-launcher-$(PLATFORM)-$(ARCH) $(BUILD_DIR)/malikclaw-launcher
	@echo "Build complete: $(BUILD_DIR)/malikclaw-launcher"
...
## build-all: Build malikclaw for all platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
...
## install: Install malikclaw to system and copy builtin skills
install: build
...
## uninstall: Remove malikclaw from system
uninstall:
...
## uninstall-all: Remove malikclaw and all data
uninstall-all:
	@echo "Removing workspace and skills..."
	@rm -rf $(MALIKCLAW_HOME)
	@echo "Removed workspace: $(MALIKCLAW_HOME)"
...
docker-build:
	...
	docker compose -f docker/docker-compose.yml build malikclaw-agent malikclaw-gateway

## docker-build-full: Build Docker image with full MCP support (Node.js 24)
docker-build-full:
	@echo "Building full-featured Docker image (Node.js 24)..."
	docker compose -f docker/docker-compose.full.yml build malikclaw-agent malikclaw-gateway
...
## docker-run: Run malikclaw gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run malikclaw gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run malikclaw agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm malikclaw-agent

## docker-run-agent-full: Run malikclaw agent in Docker (interactive, full-featured)
docker-run-agent-full:
	docker compose -f docker/docker-compose.full.yml run --rm malikclaw-agent
...
help:
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.malikclaw/workspace)"
