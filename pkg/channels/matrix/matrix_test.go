...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("malikclaw")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@malikclaw hello", want: true},
		{text: "hi @malikclaw:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bmalikclaw\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@malikclaw:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@malikclaw:matrix.org hello", want: "hello"},
		{in: "@malikclaw, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@malikclaw:matrix.org")},
				},
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body: "@malikclaw:matrix.org hello",
			},
			want: true,
		},
		{
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@malikclaw:matrix.org">MalikClaw</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40malikclaw%3Amatrix.org">MalikClaw</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@malikclaw:matrix.test"), "")
	if err != nil {
