...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.malikclaw/workspace" {
		t.Errorf("expected workspace '~/.malikclaw/workspace', got '%s'", workspace)
	}
...
func TestConvertToMalikClaw(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToMalikClaw("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.malikclaw/workspace" {
		t.Errorf("expected workspace '~/.malikclaw/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToMalikClawWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToMalikClaw("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestConvertToMalikClawWithMatrix(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestConvertToMalikClawWithMatrixDisabled(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.malikclaw/workspace" {
		t.Errorf("expected workspace '~/.malikclaw/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
