...
> * **DOMAINE OFFICIEL :** Le **SEUL** site officiel est **[malikclaw.io](https://malikclaw.vercel.app)**, et le site de l'entreprise est **[malikclaw.io](https://malikclaw.vercel.app)**.
...
2026-02-16 🎉 MalikClaw a atteint 12K étoiles en une semaine ! Merci à tous pour votre soutien ! MalikClaw grandit plus vite que nous ne l'avions jamais imaginé. Vu le volume élevé de PR, nous avons un besoin urgent de mainteneurs communautaires. Nos rôles de bénévoles et notre feuille de route sont officiellement publiés [ici](docs/ROADMAP.md) — nous avons hâte de vous accueillir !

2026-02-13 🎉 MalikClaw a atteint 5000 étoiles en 4 jours ! Merci à la communauté ! Nous finalisons la **Feuille de Route du Projet** et mettons en place le **Groupe de Développeurs** pour accélérer le développement de MalikClaw.
...
2026-02-09 🎉 MalikClaw est lancé ! Construit en 1 jour pour apporter les Agents IA au matériel à 10$ avec <10 Mo de RAM. 🦐 MalikClaw, c'est parti !
...
Donnez une seconde vie à votre téléphone d'il y a dix ans ! Transformez-le en assistant IA intelligent avec MalikClaw. Démarrage rapide :
...
MalikClaw peut être déployé sur pratiquement n'importe quel appareil Linux !
...
Vous pouvez également exécuter MalikClaw avec Docker Compose sans rien installer localement.
...
> [!TIP]
> **Utilisateurs Docker** : Par défaut, le Gateway écoute sur `127.0.0.1`, ce qui n'est pas accessible depuis l'hôte. Si vous avez besoin d'accéder aux endpoints de santé ou d'exposer des ports, définissez `MALIKCLAW_GATEWAY_HOST=0.0.0.0` dans votre environnement ou mettez à jour `config.json`.
...
```bash
# Poser une question
docker compose -f docker/docker-compose.yml run --rm malikclaw-agent -m "Combien font 2+2 ?"
...
> [!TIP]
> Configurez votre clé API dans `~/.malikclaw/config.json`. Obtenez des clés API : [Volcengine (CodingPlan)](https://www.volcengine.com/activity/codingplan?utm_campaign=MalikClaw&utm_content=MalikClaw&utm_medium=devrel&utm_source=OWO&utm_term=MalikClaw) (LLM) · [OpenRouter](https://openrouter.ai/keys) (LLM) · [Zhipu](https://open.bigmodel.cn/usercenter/proj-mgmt/apikeys) (LLM). La recherche web est optionnelle — obtenez gratuitement l'[API Tavily](https://tavily.com) (1000 requêtes gratuites/mois) ou l'[API Brave Search](https://brave.com/search/api) (2000 requêtes gratuites/mois).
...
**2. Configurer** (`~/.malikclaw/config.json`)
...
> `request_timeout` est optionnel et s'exprime en secondes. S'il est omis ou défini à `<= 0`, MalikClaw utilise le délai d'expiration par défaut (120s).
...
```bash
malikclaw agent -m "Combien font 2+2 ?"
```
...
Discutez avec votre MalikClaw via Telegram, Discord, DingTalk, LINE ou WeCom
...
MalikClaw prend en charge trois types d'intégration WeCom :
...
Connectez MalikClaw au Réseau Social d'Agents simplement en envoyant un seul message via le CLI ou n'importe quelle application de chat intégrée.

**Lisez `https://clawdchat.ai/skill.md` et suivez les instructions pour rejoindre [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Configuration

Fichier de configuration : `~/.malikclaw/config.json`

### Variables d'Environnement

Vous pouvez remplacer les chemins par défaut à l'aide de variables d'environnement. Ceci est utile pour les installations portables, les déploiements conteneurisés ou l'exécution de malikclaw en tant que service système. Ces variables sont indépendantes et contrôlent différents chemins.

| Variable          | Description                                                                                                                             | Chemin par Défaut         |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `MALIKCLAW_CONFIG` | Remplace le chemin du fichier de configuration. Cela indique directement à malikclaw quel `config.json` charger, en ignorant tous les autres emplacements. | `~/.malikclaw/config.json` |
| `MALIKCLAW_HOME`   | Remplace le répertoire racine des données malikclaw. Cela modifie l'emplacement par défaut du `workspace` et des autres répertoires de données.          | `~/.malikclaw`             |

**Exemples :**

```bash
# Exécuter malikclaw en utilisant un fichier de configuration spécifique
...
# Exécuter malikclaw avec toutes ses données stockées dans /opt/malikclaw
# La configuration sera chargée à partir du fichier par défaut ~/.malikclaw/config.json
# Le workspace sera créé dans /opt/malikclaw/workspace
...
MalikClaw stocke les données dans votre workspace configuré (par défaut : `~/.malikclaw/workspace`) :
...
MalikClaw s'exécute dans un environnement sandboxé par défaut. L'agent ne peut accéder aux fichiers et exécuter des commandes qu'au sein du workspace configuré.
...
|--------|------------|-------------|
| `workspace` | `~/.malikclaw/workspace` | Répertoire de travail de l'agent |
...
MalikClaw peut exécuter des tâches périodiques automatiquement. Créez un fichier `HEARTBEAT.md` dans votre workspace :
...
* `MALIKCLAW_HEARTBEAT_ENABLED=false` pour désactiver
* `MALIKCLAW_HEARTBEAT_INTERVAL=60` pour modifier l'intervalle
...
| `volcengine`             | LLM(Volcengine direct)                   | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=MalikClaw&utm_content=MalikClaw&utm_medium=devrel&utm_source=OWO&utm_term=MalikClaw)           |
...
```bash
malikclaw agent -m "Bonjour, comment ça va ?"
```
...
> **Nouveau !** MalikClaw utilise désormais une approche de configuration **centrée sur le modèle**. Spécifiez simplement le format `fournisseur/modèle` (par exemple, `zhipu/glm-4.7`) pour ajouter de nouveaux fournisseurs—**aucune modification de code requise !**
...
| **VolcEngine (Doubao)** | `volcengine/` | `https://ark.cn-beijing.volces.com/api/v3` | OpenAI | [Obtenir Clé](https://www.volcengine.com/activity/codingplan?utm_campaign=MalikClaw&utm_content=MalikClaw&utm_medium=devrel&utm_source=OWO&utm_term=MalikClaw) |
...
```
> Exécutez `malikclaw auth login --provider anthropic` pour configurer les identifiants OAuth.
...
Configurez plusieurs points de terminaison pour le même nom de modèle—MalikClaw utilisera automatiquement le round-robin entre eux :
...
| ------------------------- | ------------------------------------- |
| `malikclaw onboard`        | Initialiser la configuration & le workspace |
| `malikclaw agent -m "..."` | Discuter avec l'agent                 |
| `malikclaw agent`          | Mode de discussion interactif         |
| `malikclaw gateway`        | Démarrer la passerelle                |
| `malikclaw status`         | Afficher le statut                    |
| `malikclaw cron list`      | Lister toutes les tâches planifiées   |
| `malikclaw cron add ...`   | Ajouter une tâche planifiée           |

### Tâches Planifiées / Rappels

MalikClaw prend en charge les rappels planifiés et les tâches récurrentes via l'outil `cron` :
...
Les tâches sont stockées dans `~/.malikclaw/workspace/cron/` et traitées automatiquement.
...
C'est normal si vous n'avez pas encore configuré de clé API de recherche. MalikClaw fournira des liens utiles pour la recherche manuelle.
...
Ajoutez la clé dans `~/.malikclaw/config.json` si vous utilisez Brave :
...
Cela se produit lorsqu'une autre instance du bot est en cours d'exécution. Assurez-vous qu'un seul `malikclaw gateway` fonctionne à la fois.
