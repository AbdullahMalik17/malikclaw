...
<h1>MalikClaw: Trợ lý AI Siêu Nhẹ viết bằng Go</h1>
...
> * **DOMAIN CHÍNH THỨC:** Website chính thức **DUY NHẤT** là **[malikclaw.io](https://malikclaw.io)**, website công ty là **[sipeed.com](https://sipeed.com)**.
...
2026-02-16 🎉 MalikClaw đạt 12K stars chỉ trong một tuần! Cảm ơn tất cả mọi người! MalikClaw đang phát triển nhanh hơn chúng tôi tưởng tượng. Do số lượng PR tăng cao, chúng tôi cấp thiết cần maintainer từ cộng đồng. Các vai trò tình nguyện viên và roadmap đã được công bố [tại đây](docs/ROADMAP.md) — rất mong đón nhận sự tham gia của bạn!

2026-02-13 🎉 MalikClaw đạt 5000 stars trong 4 ngày! Cảm ơn cộng đồng! Chúng tôi đang hoàn thiện **Lộ trình dự án (Roadmap)** và thiết lập **Nhóm phát triển** để đẩy nhanh tốc độ phát triển MalikClaw.
...
2026-02-09 🎉 MalikClaw chính thức ra mắt! Được xây dựng trong 1 ngày để mang AI Agent đến phần cứng $10 với RAM <10MB. 🦐 MalikClaw, Lên Đường!
...
MalikClaw có thể triển khai trên hầu hết mọi thiết bị Linux!
...
Tải file binary cho nền tảng của bạn từ [trang Release](https://github.com/sipeed/malikclaw/releases).
...
Bạn cũng có thể chạy MalikClaw bằng Docker Compose mà không cần cài đặt gì trên máy.
...
> [!TIP]
> **Người dùng Docker**: Theo mặc định, Gateway lắng nghe trên `127.0.0.1`, không thể truy cập từ máy chủ. Nếu bạn cần truy cập các endpoint kiểm tra sức khỏe hoặc mở cổng, hãy đặt `MALIKCLAW_GATEWAY_HOST=0.0.0.0` trong môi trường của bạn hoặc cập nhật `config.json`.
...
```bash
# Đặt câu hỏi
docker compose -f docker/docker-compose.yml run --rm malikclaw-agent -m "2+2 bằng mấy?"
...
> [!TIP]
> Thiết lập API key trong `~/.malikclaw/config.json`. Lấy API key: [Volcengine (CodingPlan)](https://www.volcengine.com/activity/codingplan?utm_campaign=MalikClaw&utm_content=MalikClaw&utm_medium=devrel&utm_source=OWO&utm_term=MalikClaw) (LLM) · [OpenRouter](https://openrouter.ai/keys) (LLM) · [Zhipu](https://open.bigmodel.cn/usercenter/proj-mgmt/apikeys) (LLM). Tìm kiếm web là **tùy chọn** — lấy [Tavily API](https://tavily.com) miễn phí (1000 truy vấn/tháng) hoặc [Brave Search API](https://brave.com/search/api) (2000 truy vấn/tháng).
...
**2. Cấu hình** (`~/.malikclaw/config.json`)
...
> `request_timeout` là tùy chọn và dùng đơn vị giây. Nếu bỏ qua hoặc đặt `<= 0`, MalikClaw sẽ dùng timeout mặc định (120s).
...
```bash
malikclaw agent -m "Xin chào, bạn là ai?"
```
...
Trò chuyện với MalikClaw qua Telegram, Discord, DingTalk, LINE hoặc WeCom.
...
MalikClaw hỗ trợ ba loại tích hợp WeCom:
...
Kết nối MalikClaw với Mạng xã hội Agent chỉ bằng cách gửi một tin nhắn qua CLI hoặc bất kỳ ứng dụng Chat nào đã tích hợp.

**Đọc `https://clawdchat.ai/skill.md` và làm theo hướng dẫn để tham gia [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Cấu hình chi tiết

File cấu hình: `~/.malikclaw/config.json`

### Biến môi trường

Bạn có thể ghi đè các đường dẫn mặc định bằng cách sử dụng các biến môi trường. Điều này hữu ích cho việc cài đặt di động, triển khai container hóa hoặc chạy malikclaw như một dịch vụ hệ thống. Các biến này độc lập và kiểm soát các đường dẫn khác nhau.

| Biến              | Mô tả                                                                                                                             | Đường dẫn mặc định        |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `MALIKCLAW_CONFIG` | Ghi đè đường dẫn đến file cấu hình. Điều này trực tiếp yêu cầu malikclaw tải file `config.json` nào, bỏ qua tất cả các vị trí khác. | `~/.malikclaw/config.json` |
| `MALIKCLAW_HOME`   | Ghi đè thư mục gốc cho dữ liệu malikclaw. Điều này thay đổi vị trí mặc định của `workspace` và các thư mục dữ liệu khác.          | `~/.malikclaw`             |

**Ví dụ:**

```bash
# Chạy malikclaw bằng một file cấu hình cụ thể
...
# Chạy malikclaw với tất cả dữ liệu được lưu trữ trong /opt/malikclaw
# Cấu hình sẽ được tải từ ~/.malikclaw/config.json mặc định
# Workspace sẽ được tạo tại /opt/malikclaw/workspace
...
MalikClaw lưu trữ dữ liệu trong workspace đã cấu hình (mặc định: `~/.malikclaw/workspace`):
...
MalikClaw chạy trong môi trường sandbox theo mặc định. Agent chỉ có thể truy cập file và thực thi lệnh trong phạm vi workspace.
...
|----------|---------|-------|
| `workspace` | `~/.malikclaw/workspace` | Thư mục làm việc của agent |
...
MalikClaw có thể tự động thực hiện các tác vụ định kỳ. Tạo file `HEARTBEAT.md` trong workspace:
...
* `MALIKCLAW_HEARTBEAT_ENABLED=false` để tắt
* `MALIKCLAW_HEARTBEAT_INTERVAL=60` để thay đổi khoảng thời gian
...
| `volcengine`             | LLM(Volcengine trực tiếp)                   | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=MalikClaw&utm_content=MalikClaw&utm_medium=devrel&utm_source=OWO&utm_term=MalikClaw)           |
...
> **Tính năng mới!** MalikClaw hiện sử dụng phương pháp cấu hình **đặt mô hình vào trung tâm**. Chỉ cần chỉ định dạng `nhà cung cấp/mô hình` (ví dụ: `zhipu/glm-4.7`) để thêm nhà cung cấp mới—**không cần thay đổi mã!**
...
| **VolcEngine (Doubao)** | `volcengine/` | `https://ark.cn-beijing.volces.com/api/v3` | OpenAI | [Lấy Khóa](https://www.volcengine.com/activity/codingplan?utm_campaign=MalikClaw&utm_content=MalikClaw&utm_medium=devrel&utm_source=OWO&utm_term=MalikClaw) |
...
```
> Chạy `malikclaw auth login --provider anthropic` để thiết lập thông tin xác thực OAuth.
...
Định cấu hình nhiều endpoint cho cùng một tên mô hình—MalikClaw sẽ tự động phân phối round-robin giữa chúng:
...
| Lệnh | Mô tả |
| --- | --- |
| `malikclaw onboard` | Khởi tạo cấu hình & workspace |
| `malikclaw agent -m "..."` | Trò chuyện với agent |
| `malikclaw agent` | Chế độ chat tương tác |
| `malikclaw gateway` | Khởi động gateway (cho bot chat) |
| `malikclaw status` | Hiển thị trạng thái |
| `malikclaw cron list` | Liệt kê tất cả tác vụ định kỳ |
| `malikclaw cron add ...` | Thêm tác vụ định kỳ |

### Tác vụ định kỳ / Nhắc nhở

MalikClaw hỗ trợ nhắc nhở theo lịch và tác vụ lặp lại thông qua công cụ `cron`:
...
Các tác vụ được lưu trong `~/.malikclaw/workspace/cron/` và được xử lý tự động.
...
Điều này là bình thường nếu bạn chưa cấu hình API key cho tìm kiếm. MalikClaw sẽ cung cấp các liên kết hữu ích để tìm kiếm thủ công.
...
Thêm key vào `~/.malikclaw/config.json` nếu dùng Brave:
...
Điều này xảy ra khi có một instance bot khác đang chạy. Đảm bảo chỉ có một tiến trình `malikclaw gateway` chạy tại một thời điểm.
