	...
	items = append(items,
		MenuItem{
			...
			Action: func() {
				...
				s.addModel(
					malikclawconfig.ModelConfig{ModelName: newName, Model: "openai/gpt-5.4"},
				)
				s.push(
...
func (s *appState) addModel(model malikclawconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []malikclawconfig.ModelConfig) {
...
func isModelValid(model malikclawconfig.ModelConfig) bool {
...
func (s *appState) testModel(model *malikclawconfig.ModelConfig) {
	if model == nil {
		return
	}
