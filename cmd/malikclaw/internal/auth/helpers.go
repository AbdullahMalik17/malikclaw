...
const (
	supportedProvidersMsg = "supported providers: openai, anthropic, google-antigravity, google-gmail"
...
func authLoginCmd(provider string, useDeviceCode bool, useOauth bool) error {
	...
	case "google-gmail", "gmail":
		return authLoginGoogleGmail()
	default:
		return fmt.Errorf("unsupported provider: %s (%s)", provider, supportedProvidersMsg)
	}
}

func authLoginGoogleGmail() error {
	// For Gmail, we need ClientID and ClientSecret from the user.
	// We check environment variables first, then prompt.
	clientID := os.Getenv("MALIKCLAW_GMAIL_CLIENT_ID")
	clientSecret := os.Getenv("MALIKCLAW_GMAIL_CLIENT_SECRET")

	if clientID == "" || clientSecret == "" {
		fmt.Println("Gmail API requires your own Google Cloud Client ID and Client Secret.")
		fmt.Println("Set them via MALIKCLAW_GMAIL_CLIENT_ID and MALIKCLAW_GMAIL_CLIENT_SECRET env vars,")
		fmt.Println("or enter them below:")
	...
	if clientID == "" || clientSecret == "" {
		return fmt.Errorf("Client ID and Client Secret are required for Gmail login")
	}
	...
	if err = auth.SetCredential("google-gmail", cred); err != nil {
		return fmt.Errorf("failed to save credentials: %w", err)
	}

	fmt.Println("\n✓ Google Gmail login successful!")
	fmt.Println("You can now use the 'gmail' tool.")
...
func authLoginGoogleAntigravity() error {
	...
	fmt.Println("Try it: malikclaw agent -m \"Hello world\"")
...
func authStatusCmd() error {
	...
	if len(store.Credentials) == 0 {
		...
		fmt.Println("Run: malikclaw auth login --provider <name>")
		return nil
	}
...
func authModelsCmd() error {
	...
	if err != nil || cred == nil {
		return fmt.Errorf(
			"not logged in to Google Antigravity.\nrun: malikclaw auth login --provider google-antigravity",
		)
	}
