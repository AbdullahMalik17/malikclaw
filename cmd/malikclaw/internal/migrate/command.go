...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to malikclaw",
		Args:  cobra.NoArgs,
		Example: `  malikclaw migrate
  malikclaw migrate --from openclaw
  malikclaw migrate --dry-run
  malikclaw migrate --refresh
  malikclaw migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.malikclaw)")
