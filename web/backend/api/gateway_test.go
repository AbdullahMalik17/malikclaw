...
func TestGatewayRestartReturnsErrorStatusWhenReplacementFailsToStart(t *testing.T) {
	...
	invalidBinaryPath := filepath.Join(t.TempDir(), "fake-malikclaw")
	if err := os.WriteFile(invalidBinaryPath, []byte("#!/bin/sh\n"), 0o644); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	t.Setenv("MALIKCLAW_BINARY", invalidBinaryPath)
...
func TestFindMalikclawBinary_EnvOverride(t *testing.T) {
	// Create a temporary file to act as the mock binary
	tmpDir := t.TempDir()
	mockBinary := filepath.Join(tmpDir, "malikclaw-mock")
	if err := os.WriteFile(mockBinary, []byte("mock"), 0o755); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}

	t.Setenv("MALIKCLAW_BINARY", mockBinary)

	got := utils.FindMalikclawBinary()
	if got != mockBinary {
		t.Errorf("FindMalikclawBinary() = %q, want %q", got, mockBinary)
	}
}

func TestFindMalikclawBinary_EnvOverride_InvalidPath(t *testing.T) {
	// When MALIKCLAW_BINARY points to a non-existent path, fall through to next strategy
	t.Setenv("MALIKCLAW_BINARY", "/nonexistent/malikclaw-binary")

	got := utils.FindMalikclawBinary()
	// Should not return the invalid path; falls back to "malikclaw" or another found path
	if got == "/nonexistent/malikclaw-binary" {
		t.Errorf("FindMalikclawBinary() returned invalid env path %q, expected fallback", got)
	}
}
