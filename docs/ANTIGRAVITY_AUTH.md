...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in MalikClaw.
...
10. [Creating a New Provider in MalikClaw](#creating-a-new-provider-in-malikclaw)
...
const antigravityPlugin = {
  ...
  register(api: MalikClawPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<MalikClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- MalikClaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.malikclaw/auth.json`:
...
## Creating a New Provider in MalikClaw

MalikClaw providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/malikclaw/cmd_auth.go`:
...
malikclaw auth login --provider your-provider

# List models (for Antigravity)
malikclaw auth models

# Start the gateway
malikclaw gateway

# Run an agent with a specific model
malikclaw agent -m "Hello" --model your-model
```
...
export MALIKCLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export MALIKCLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `malikclaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.malikclaw/auth.json`
- Re-run `malikclaw auth login --provider antigravity`
