...
<h1>MalikClaw: Go で書かれた超効率 AI アシスタント</h1>
...
## 📢 ニュース
2026-02-09 🎉 MalikClaw リリース！$10 ハードウェアで 10MB 未満の RAM で動く AI エージェントを 1 日で構築。🦐 行くぜ、シャコ！
...
### 🐜 革新的な省フットプリントデプロイ
MalikClaw はほぼすべての Linux デバイスにデプロイできます！
...
Docker Compose を使えば、ローカルにインストールせずに MalikClaw を実行できます。
...
> [!TIP]
> **Docker ユーザー**: デフォルトでは、Gateway は `127.0.0.1` でリッスンしており、ホストからアクセスできません。ヘルスチェックエンドポイントにアクセスしたり、ポートを公開したりする必要がある場合は、環境変数で `MALIKCLAW_GATEWAY_HOST=0.0.0.0` を設定するか、`config.json` を更新してください。
...
> [!TIP]
> `~/.malikclaw/config.json` に API キーを設定してください。API キーの取得先: [Volcengine (CodingPlan)](https://www.volcengine.com/activity/codingplan?utm_campaign=MalikClaw&utm_content=MalikClaw&utm_medium=devrel&utm_source=OWO&utm_term=MalikClaw) (LLM) · [OpenRouter](https://openrouter.ai/keys) (LLM) · [Zhipu](https://open.bigmodel.cn/usercenter/proj-mgmt/apikeys) (LLM)。Web 検索は **任意** です — 無料の [Tavily API](https://tavily.com) (月 1000 クエリ無料) または [Brave Search API](https://brave.com/search/api) (月 2000 クエリ無料)。
...
> `request_timeout` は任意の秒単位設定です。省略または `<= 0` の場合、MalikClaw はデフォルトのタイムアウト（120秒）を使用します。
...
Telegram、Discord、QQ、DingTalk、LINE、WeCom で MalikClaw と会話できます
...
> **Docker Compose**: Gateway HTTP サーバーは共有の `127.0.0.1:18790` で Webhook を提供します。ホストからアクセスするには `malikclaw-gateway` サービスに `ports: ["18790:18790"]` を追加してください。
...
MalikClaw は3種類の WeCom 統合をサポートしています：
...
設定ファイル: `~/.malikclaw/config.json`

### 環境変数

環境変数を使用してデフォルトのパスを上書きできます。これは、ポータブルインストール、コンテナ化されたデプロイメント、または malikclaw をシステムサービスとして実行する場合に便利です。これらの変数は独立しており、異なるパスを制御します。

| 変数              | 説明                                                                                                                             | デフォルトパス            |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `MALIKCLAW_CONFIG` | 設定ファイルへのパスを上書きします。これにより、malikclaw は他のすべての場所を無視して、指定された `config.json` をロードします。 | `~/.malikclaw/config.json` |
| `MALIKCLAW_HOME`   | malikclaw データのルートディレクトリを上書きします。これにより、`workspace` やその他のデータディレクトリのデフォルトの場所が変更されます。          | `~/.malikclaw`             |

**例：**

```bash
# 特定の設定ファイルを使用して malikclaw を実行する
# ワークスペースのパスはその設定ファイル内から読み込まれます
MALIKCLAW_CONFIG=/etc/malikclaw/production.json malikclaw gateway

# すべてのデータを /opt/malikclaw に保存して malikclaw を実行する
# 設定はデフォルトの ~/.malikclaw/config.json からロードされます
# ワークスペースは /opt/malikclaw/workspace に作成されます
...
MalikClaw は設定されたワークスペース（デフォルト: `~/.malikclaw/workspace`）にデータを保存します：
...
MalikClaw はデフォルトでサンドボックス環境で実行されます。エージェントは設定されたワークスペース内のファイルにのみアクセスし、コマンドを実行できます。
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `workspace` | `~/.malikclaw/workspace` | エージェントの作業ディレクトリ |
...
MalikClaw は自動的に定期タスクを実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成します：
...
**環境変数:**
- `MALIKCLAW_HEARTBEAT_ENABLED=false` で無効化
- `MALIKCLAW_HEARTBEAT_INTERVAL=60` で間隔変更
...
| `volcengine`             | LLM(Volcengine 直接)                   | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=MalikClaw&utm_content=MalikClaw&utm_medium=devrel&utm_source=OWO&utm_term=MalikClaw)           |
...
> **新機能！** MalikClaw は現在 **モデル中心** の設定アプローチを採用しています。`ベンダー/モデル` 形式（例: `zhipu/glm-4.7`）を指定するだけで、新しいプロバイダーを追加できます—**コードの変更は一切不要！**
...
| **VolcEngine (Doubao)** | `volcengine/` | `https://ark.cn-beijing.volces.com/api/v3` | OpenAI | [キーを取得](https://www.volcengine.com/activity/codingplan?utm_campaign=MalikClaw&utm_content=MalikClaw&utm_medium=devrel&utm_source=OWO&utm_term=MalikClaw) |
...
```
> OAuth認証を設定するには、`malikclaw auth login --provider anthropic` を実行してください。
...
同じモデル名で複数のエンドポイントを設定すると、MalikClaw が自動的にラウンドロビンで分散します：
...
| コマンド | 説明 |
|---------|------|
| `malikclaw onboard` | 設定＆ワークスペースの初期化 |
| `malikclaw agent -m "..."` | エージェントとチャット |
| `malikclaw agent` | インタラクティブチャットモード |
| `malikclaw gateway` | ゲートウェイを起動 |
| `malikclaw status` | ステータスを表示 |
...
検索 API キーをまだ設定していない場合、これは正常です。MalikClaw は手動検索用の便利なリンクを提供します。
...
別のインスタンスが実行中の場合に発生します。`malikclaw gateway` が 1 つだけ実行されていることを確認してください。
